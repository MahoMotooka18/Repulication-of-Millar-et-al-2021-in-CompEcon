...
class KSExperimentRunnerComplete:
        ...
        for period in range(total_periods):
            ...
            if period % train_every == 0:
                ...
                if objective_name == 'lifetime_reward':
                    ...
                    raise ValueError(f"Unknown objective: {objective_name}. Must be one of: lifetime_reward, euler, bellman")
    ...
    def _train_step_lifetime_reward(
        ...
        for _ in range(T):
            ...
            # Diagnostic check: Ensure consumption is feasible
            violation_ratio = (c_all > w_t).float().mean().item()
            if violation_ratio > 0.01:  # If >1% of samples violate constraint
                import sys
                print(f"[WARNING] Consumption constraint violation: {violation_ratio*100:.1f}%", file=sys.stderr)
                print(f"  c_all stats: min={c_all.min():.6f}, max={c_all.max():.6f}, mean={c_all.mean():.6f}", file=sys.stderr)
                print(f"  w_t stats: min={w_t.min():.6f}, max={w_t.max():.6f}, mean={w_t.mean():.6f}", file=sys.stderr)
                print(f"  Constraint (c <= w): mean_ratio = {(c_all/torch.clamp(w_t, min=1e-6)).mean():.4f}", file=sys.stderr)
